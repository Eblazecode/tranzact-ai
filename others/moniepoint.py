...
def moniepoint_preprocessing():
    ...
    KEYWORDS = {
        ...
        "Electricity": [
            ...
            'IKEDC', 'IKEJA ELECTRIC', 'PHED', 'PORT HARCOURT ELECTRICITY',
    ...
    def clean_description(narration, tref, category, debit, credit):
        ...
        if category == "Electricity":
            DISCO_NAMES = {
                ...
                'IKEDC': 'IKEDC', 'IKEJA ELECTRIC': 'IKEDC',
    ...
    def load_file(file_path):
        ...
        if ext == '.csv':
            ...
            for _, row in header_df.iterrows():
                for i, val in enumerate(row):
                    if str(val).strip().upper() == "ACCOUNT NAME:":
                        name = str(row.iloc[i + 2]).strip()
        ...
        return account_name, user_name, df
    ...
    file_path = filedialog.askopenfilename(
        title="Select Moniepoint Statement (CSV or Excel)",
        filetypes=[
    ...
    try:
        account_name, user_name, df = load_file(file_path)
        ...
        # Add User and Bank columns to match OPay output format
        final_df['User'] = user_name
        final_df['Bank'] = 'Moniepoint'

        save_path = Path.home() / "Downloads" / f"{account_name}_Moniepoint_Preprocessed.xlsx"
        final_df.to_excel(save_path, index=False)
        messagebox.showinfo("Success", f"Moniepoint Preprocessing Complete!\nSaved to: {save_path}")
