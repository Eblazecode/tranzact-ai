...
def zenith_preprocessing():
    ...
    KEYWORDS = {
        ...
        "Church": [
            ...
            'EVANGELICAL', 'PENTECOSTAL', 'BAPTIST', 'CATHOLIC', 'ANGLICAN',
            'RCCG', 'WINNERS', 'DEEPER LIFE', 'SALVATION', 'MOUNTAIN OF FIRE',
            'CHRIST EMBASSY', 'DAYSTAR', 'LIVING FAITH', 'SANCTUARY',
            'WORSHIP CENTRE', 'TITHE', 'OFFERING'
        ],
        "Utilities": [
    ...
    # Subscription platforms — only used when MC POS INTL is in the text
    ...
    file_path = filedialog.askopenfilename(
        title="Select Zenith Bank PDF Statement",
    ...
    try:
        ...
        if not rows:
            messagebox.showerror("Error", "No transaction data found in PDF.")
            return
        ...
        output_filename = f"{account_name}_Zenith_Preprocessed.xlsx"
        save_path = Path.home() / "Downloads" / output_filename

        final_df.to_excel(save_path, index=False)

        messagebox.showinfo(
            "Success",
            f"Zenith Bank Preprocessing Complete!\n\n"
            f"Account Holder : {account_name.replace('_', ' ')}\n"
