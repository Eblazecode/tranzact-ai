...
def gtbank_preprocessing():
    ...
    file_path = filedialog.askopenfilename(
        title='Select GTBank PDF Statement',
    ...
    try:
        ...
        output_filename = f'{account_name}_GTBank_Preprocessed.xlsx'
        save_path       = Path(__file__).parent.parent / 'backend' / 'data' / 'uploads' / output_filename
        save_path.parent.mkdir(parents=True, exist_ok=True)
        final_df.to_excel(save_path, index=False)

        total         = len(final_df)
        uncategorised = (final_df['Category'] == 'Others').sum()
        messagebox.showinfo(
            'Success',
            f'GTBank Preprocessing Complete!\n\n'
